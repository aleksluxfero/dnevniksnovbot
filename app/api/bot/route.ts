...
bot.on("message:voice", async (ctx) => {
  try {
    ...
    try {
      const prompt = `
        Извлеки из текста основные теги, следуя этим правилам:
        1. Используй только существительные в единственном числе.
        2. Применяй правила русского языка для склонения слов и имен фамилий.
        3. Объединяй двойные слова в одно слово (например, "тетя Аня" → #тетяАня).
        4. Имена собственные сохраняй с большой буквы, остальное — с маленькой.
        5. Верни ТОЛЬКО теги через пробел, каждый с #.

        Примеры:
        Входной текст: "Еду с батей на машине по лесу"
        Выход: "#батя #машина #лес"
        
        Входной текст: "Снится что мы с тетей Аней идем по лесу в розовых трусах"
        Выход: "#тетяАня #ходьба #лес #трусы"
        
        Входной текст: "Снится мне что мы с Аскаровыми идем"
        Выход: "#аскаровы #ходьба"
